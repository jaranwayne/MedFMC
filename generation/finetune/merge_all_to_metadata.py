...
def merge_metadata(image_paths, metadata, full_path, tags_dir, captions_dir):
    for image_path in tqdm(image_paths):
        ...
        tags_path = Path(tags_dir) / (image_path.stem + TAGS_EXT)
        caption_path = Path(captions_dir) / (image_path.stem + CAPTION_EXT)
...
def main(args):
    ...
    metadata = merge_metadata(image_paths, metadata, args.full_path, args.tags_dir, args.captions_dir)
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--in_json", type=str, help="metadata file to input (if omitted and out_json exists, existing out_json is read)")
    parser.add_argument("--full_path", action="store_true", help="use full path as image-key in metadata (supports multiple directories)")
    parser.add_argument("--recursive", action="store_true", help="recursively search for training tags and captions in all child folders of train_data_dir")
    parser.add_argument("--debug", action="store_true", help="debug mode")
    parser.add_argument("--clean_caption", action="store_true", help="clean captions and tags in metadata")
    parser.add_argument("--tags_dir", type=str, help="directory for tags files")
    parser.add_argument("--captions_dir", type=str, help="directory for captions files")
