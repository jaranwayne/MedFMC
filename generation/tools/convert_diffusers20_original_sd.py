...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--v1", action="store_true", help="load v1.x model (v1 or v2 is required to load checkpoint) "
    )
    parser.add_argument(
        "--v2", action="store_true", help="load v2.0 model (v1 or v2 is required to load checkpoint) "
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--fp16",
        action="store_true",
        help="load as fp16 (Diffusers only) and save as fp16 (checkpoint only)",
    )
    parser.add_argument("--bf16", action="store_true", help="save as bf16 (checkpoint only) ")
    parser.add_argument(
        "--float", action="store_true", help="save as float (checkpoint only) "
    )
    parser.add_argument(
        "--save_precision_as",
        type=str,
        ...
        help="save precision, do not specify with --fp16/--bf16/--float",
    )
    parser.add_argument("--epoch", type=int, default=0, help="epoch to write to checkpoint ")
    parser.add_argument(
        "--global_step", type=int, default=0, help="global_step to write to checkpoint "
    )
    parser.add_argument(
        "--reference_model",
        type=str,
    ...
    parser.add_argument(
        "--use_safetensors",
        action="store_true",
        help="use safetensors format to save Diffusers model (checkpoint depends on the file extension) ",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="model to load: checkpoint file or Diffusers model's directory ",
    )
    parser.add_argument(
        "model_to_save",
        type=str,
        default=None,
        help="model to save: checkpoint (with extension) or Diffusers model's directory (without extension) ",
    )
    return parser
